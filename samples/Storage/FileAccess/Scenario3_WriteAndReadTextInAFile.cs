...
namespace FileAccess
{
    public class Scenario3_WriteAndReadTextInAFile
    {
        ...
        {
            Debug.WriteLine($"== Scenario3_WriteAndReadTextInAFile ==");
            ...
            {
                Debug.WriteLine($"ERROR: write operation on file failed.");
            }
            ...
            {
                Debug.WriteLine($"ERROR: read operation on file failed.");
            }

            // compare
            if(textContent.GetHashCode() == textFromFile.GetHashCode())
            {
                Debug.WriteLine($"OK: read text matches written text.");
            }
            else
            {
                Debug.WriteLine($"ERROR: read text does not match written text.");
            }
        }
    }
}
